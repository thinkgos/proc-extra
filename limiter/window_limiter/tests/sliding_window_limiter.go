...
const testSlidingWindowLimiterInvalidKind = "invalid_kind"
const testSlidingWindowLimiterKind = "test_kind"

const (
	testSlidingWindowLimiterKeyPrefix = "period:limiter:test-kind:"
...
func GenericTest_SlidingWindowLimiter_InvalidKind[B window_limiter.SlidingWindowLimiterBackend](t *testing.T, mr *miniredis.Miniredis, backend B) {
	l := window_limiter.NewSlidingWindowLimiter[string](backend).
		SetParams(testSlidingWindowLimiterParams)

	_, err := l.Take(context.Background(), testSlidingWindowLimiterInvalidKind, testSlidingWindowLimiterId1)
	require.ErrorIs(t, window_limiter.ErrParamKindNotFound, err)

	_, err = l.Check(context.Background(), testSlidingWindowLimiterInvalidKind, testSlidingWindowLimiterId1)
	require.ErrorIs(t, window_limiter.ErrParamKindNotFound, err)

	_, err = l.Lock(context.Background(), testSlidingWindowLimiterInvalidKind, testSlidingWindowLimiterId1)
	require.ErrorIs(t, window_limiter.ErrParamKindNotFound, err)

	err = l.Reset(context.Background(), testSlidingWindowLimiterInvalidKind, testSlidingWindowLimiterId1)
	require.ErrorIs(t, window_limiter.ErrParamKindNotFound, err)
}
...
func GenericTest_SlidingWindowLimiter_Work[B window_limiter.SlidingWindowLimiterBackend](t *testing.T, mr *miniredis.Miniredis, backend B) {
	...
	pv1, err := l.Check(context.Background(), testSlidingWindowLimiterKind, testSlidingWindowLimiterId1)
	...
	for i := range testSlidingWindowLimiterMaxLimit {
		v, err := l.Take(context.Background(), testSlidingWindowLimiterKind, testSlidingWindowLimiterId1)
		require.NoError(t, err)
	...
	pv2, err := l.Check(context.Background(), testSlidingWindowLimiterKind, testSlidingWindowLimiterId1)
	require.NoError(t, err)
	...
	err = l.Reset(context.Background(), testSlidingWindowLimiterKind, testSlidingWindowLimiterId1)
	require.NoError(t, err)

	// peek the sliding window after reset
	pv3, err := l.Check(context.Background(), testSlidingWindowLimiterKind, testSlidingWindowLimiterId1)
	require.NoError(t, err)
...
func GenericTest_SlidingWindowLimiter_Lock[B window_limiter.SlidingWindowLimiterBackend](t *testing.T, mr *miniredis.Miniredis, backend B) {
	...
	v, err = l.Lock(context.Background(), testSlidingWindowLimiterKind, testSlidingWindowLimiterId1)
	require.NoError(t, err)
	require.False(t, v.Allow)
	require.Equal(t, 3, v.Count)
	require.Equal(t, testSlidingWindowLimiterMaxLimit, v.MaxLimit)
